...
export default function Article2() {
  ...
  const t = {
    ...
    title: isAr
      ? 'العثور على "المناطق الذهبية" في الرياض: كيف تحدد المواقع عالية الربح في 2026'
    ...
    sec1P1Start: isAr
      ? 'في سوق العقارات السعودي لعام 2026، "الشارع المزدحم" لا يضمن الربح. مع تدفق المستثمرين الأجانب ولوائح الإيجار الجديدة، يبحث أذكى أصحاب الأعمال عن '
    ...
    sec1P1Mid: isAr
      ? ' — جيوب مخفية في الرياض وجدة حيث القوة الشرائية عالية، لكن عرض الخدمات منخفض بشكل حرج. لا يمكنك العثور على هذه المناطق بالتجول بالسيارة؛ بل تجدها باستخدام '
      : ' — hidden pockets in Riyadh and Jeddah where purchasing power is high, but service supply is critically low. You cannot find these zones by driving around; you find them using ',
    ...
    sec2P2End: isAr
      ? ': حركة مرور عالية، لكنها مشبعة بأكثر من 50 منافساً وإيجارات متضخمة.'
      : ': High traffic, but oversaturated with 50+ competitors and inflated rents.',
    sec2P3: isAr
      ? 'تحدد S-LOC (S-Locator) البديل الأمثل. تقوم خوارزمياتنا بمسح شبكة المدينة لإجراء "تحليل الفجوات". نحن نسلط الضوء على مناطق محددة حيث:'
      : 'S-LOC (S-Locator) identifies the alternative. Our algorithms scan the city grid to perform “Gap Analysis.” We highlight specific districts where:',

    sec2List: isAr
      ? [
          {
            b: 'الكثافة السكانية في ازدياد (هيئة الإحصاء):',
            t: ' المجمعات السكنية الجديدة تمتلئ.',
          },
          {
            b: 'الرخص التجارية منخفضة (وزارة التجارة):',
            t: ' يوجد عدد قليل جداً من التصاريح الحالية لقطاعك المحدد (مثل "القهوة المختصة" أو "مطاعم العائلات").',
          },
          { b: 'الإيجار مقوم بأقل من قيمته:', t: ' سعر الإيجار الحالي أقل من الإيرادات المتوقعة.' },
        ]
      : [
          {
            b: 'Population Density is Rising (GASTAT):',
            t: ' New residential compounds are filling up.',
          },
          {
            b: 'Commercial Licenses are Low (MOC):',
            t: ' There are very few existing permits for your specific sector (e.g., “Specialty Coffee” or “Family Dining”).',
          },
          {
            b: 'Rent is Undervalued:',
            t: ' The current rental price is lower than the predicted revenue.',
          },
        ],

    section3Title: isAr ? 'الدليل (السلطة والثقة)' : 'The Evidence (Authority & Trust)',
    sec3P1: isAr
      ? 'لتأكيد أن المنطقة "ذهبية" حقاً، تقوم S-LOC بمقاطعة ثلاث مجموعات بيانات رسمية للتحقق من صحة الاستثمار:'
      : 'To confirm a zone is truly “Golden,” S-LOC cross-references three official datasets to validate the investment:',

    sec3List: isAr
      ? [
          {
            b: 'التحقق من القوة الشرائية:',
            t: ' نستخدم بيانات فئات الدخل من هيئة الإحصاء لتأكيد قدرة الحي على شراء منتجك (مثل التمييز بين المناطق "الفاخرة" و"الاقتصادية").',
          },
          {
            b: 'تأثير استقرار الإيجارات:',
            t: ' مع تطبيق تدابير استقرار الإيجارات في الرياض، فإن تأمين عقد إيجار طويل الأجل في "منطقة ذهبية" يثبت الآن تكاليفك التشغيلية بينما تنمو إيراداتك. تتحقق S-LOC من سجلات معاملات وزارة العدل للتأكد من أنك لا تدخل منطقة بأسعار مرتفعة بشكل مصطنع.',
          },
          {
            b: 'مسار النمو المستقبلي:',
            t: ' نقوم بتركيب خرائط مترو وحافلات الرياض للتنبؤ بزيادات حركة المشاة المستقبلية قبل حدوثها.',
          },
        ]
      : [
          {
            b: 'Spending Power Validation:',
            t: ' We use GASTAT income bracket data to confirm the neighborhood can afford your product (e.g., distinguishing between “Luxury” vs. “Economy” zones).',
          },
          {
            b: 'Rent Freeze Impact:',
            t: " With the Riyadh rent stabilization measures in effect, securing a long-term lease in a “Golden Zone” now locks in your operational costs while your revenue grows. S-LOC checks MOJ transaction records to ensure you aren't entering a district with artificially spiked prices.",
          },
          {
            b: 'Future Growth Trajectory:',
            t: ' We overlay Riyadh Metro and Bus Network maps to predict future footfall spikes before they happen.',
          },
        ],
    ...
    sec4P1End: isAr
      ...
      : ' Open the heatmap, filter by your industry, and instantly see where the market gaps are in Riyadh.',
    ...
    <div className="w-full bg-[#fafbfc] font-sans overflow-hidden" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <section className="max-w-3xl mx-auto px-4 py-20">
        ...
        <div className={`text-gray-700 leading-loose text-lg ${isAr ? 'text-right' : 'text-left'}`}>
          ...
          <p className="mb-6">
            {t.sec1P1Start}
            <strong className="text-[#2b1055] font-black">{t.sec1P1Bold}</strong>
            {t.sec1P1Mid}
            <strong>{t.sec1P1Bold2}</strong>
            {t.sec1P1End}
          </p>
          ...
          <p className="mb-4">
            {t.sec2P1Start}
            <strong className="text-[#046bd2]">{t.sec2P1Bold}</strong>
          </p>
          <p className="mb-4">
            {t.sec2P2Start}
            <strong>{t.sec2P2Bold}</strong>
            {t.sec2P2End}
          </p>
          <p className="mb-4">
            <strong>{isAr ? 'S-LOC' : 'S-LOC (S-Locator)'}</strong>{' '}
            {t.sec2P3.replace('S-LOC (S-Locator) ', '')}
          </p>

          <ol
            className={`list-decimal mb-8 space-y-4 font-medium text-gray-600 marker:text-[#38e54d] marker:font-bold ${isAr ? 'pr-6' : 'pl-6'}`}
          >
          ...
          <ul
            className={`list-disc mb-8 space-y-4 font-medium text-gray-600 marker:text-[#38e54d] ${isAr ? 'pr-6' : 'pl-6'}`}
          >
            {t.sec3List.map((item, index) => (
              <li key={index}>
                <strong className="text-[#110222]">{item.b}</strong>
                {item.t}
              </li>
            ))}
          </ul>
