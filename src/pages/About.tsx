    ...
    <div className="bg-[#1f0f38] border border-[#38e54d]/20 mb-4 shadow-lg rounded-xl overflow-hidden transition-all duration-300 hover:border-[#38e54d]/60 group">
      ...
      >
        <div className="flex items-center gap-5">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors duration-300 ${isOpen ? 'bg-[#38e54d] text-[#110222]' : 'bg-white/5 text-[#38e54d] group-hover:bg-[#38e54d]/20'}`}
          >
            <span className="text-2xl font-black leading-none pb-1">{isOpen ? '-' : '+'}</span>
          </div>
          <span
            className={`text-[16px] font-bold text-${isAr ? 'right' : 'left'} transition-colors duration-300 ${isOpen ? 'text-[#38e54d]' : 'text-white group-hover:text-[#38e54d]'}`}
          >
            {title}
          </span>
        </div>
      ...
      >
        <div
          className={`px-6 pb-8 pt-0 text-gray-300 text-[15px] leading-relaxed space-y-3 ${isAr ? 'sm:pr-[84px]' : 'sm:pl-[84px]'}`}
        >
          {children}
        </div>
      </div>
...
export default function About() {
  ...
  const t = {
    ...
    heroDesc: isAr
      ? 'اكتشف من نحن، وما يحفزنا، وكيف نمكّن الشركات من اتخاذ قرارات أكثر ذكاءً بدقة وذكاء البيانات.'
    ...
    teamDesc: isAr
      ? 'تعرف على فريقنا من المحترفين المتفانين، الذين يجمعون بين الخبرة والإبداع والشغف لتقديم نتائج استثنائية ودفع عجلة نجاحك.'
    ...
    faq1Title: isAr
      ? 'أريد إجراء دراسة جدوى لمقهى في السعودية'
      : 'I want to do feasibility study for cafe in Saudi Arabia',
    faq1State: isAr ? 'البيان:' : 'Statement:',
    faq1StateText: isAr
      ? 'الطريقة الأكثر كفاءة لإجراء دراسة جدوى لمقهى في السعودية هي استخدام ذكاء المواقع بدلاً من الاستطلاعات اليدوية.'
      : 'The most efficient way to conduct a feasibility study for a cafe in Saudi Arabia is to use Location Intelligence rather than manual surveys.',
    faq1Expl: isAr ? 'الشرح:' : 'Explanation:',
    faq1ExplText: isAr
      ? 'تستغرق دراسات الجدوى التقليدية للمقاهي 3-4 أسابيع وتعتمد على العد اليدوي. بينما تقوم المنصات الحديثة مثل S-LOC برقمنة هذه العملية باستخدام البيانات الجغرافية المكانية لتحليل التركيبة السكانية والقوة الشرائية وسلوكيات استهلاك القهوة.'
      : 'Traditional cafe feasibility studies take 3-4 weeks and rely on manual footfall counting. Modern platforms like S-LOC (S-Locator) digitize this process, using geospatial data to analyze demographics, spending power, and coffee consumption behaviors in specific neighborhoods.',
    faq1Evid: isAr ? 'الدليل:' : 'Evidence:',
    faq1EvidText: isAr
      ? 'تجمع S-LOC البيانات الرسمية من الهيئة العامة للإحصاء ووزارة العدل لإنشاء توقعات مالية شاملة في أقل من 60 ثانية.'
      : 'S-LOC aggregates official data from GASTAT (for population density) and the Ministry of Justice (for commercial rental indices) to generate a comprehensive financial forecast in under 60 seconds.',
    faq1Act: isAr ? 'الإجراء:' : 'Action:',
    faq1ActLink: isAr
      ? '[ابدأ دراسة جدوى مقهاك مع S-LOC]'
      : '[Start your Cafe Feasibility Study with S-LOC]',
    faq1ActText: isAr
      ? 'للحصول على تقرير فوري خلال 15 دقيقة بدلاً من الانتظار لأسابيع.'
      : 'to get a 15-minute instant report instead of waiting weeks.',

    faq2Title: isAr
      ? 'أريد إجراء تحليل موقع لصيدلية في الرياض'
      : 'I want to do site analysis for a pharmacy in Riyadh',
    faq2StateText: isAr
      ? 'يتطلب التحليل الناجح لموقع صيدلية في الرياض تحديد "المناطق الذهبية" — وهي مناطق ذات كثافة سكانية عالية ولكن بتشبع منخفض للمنافسين.'
      : 'Successful site analysis for a pharmacy in Riyadh requires identifying "Golden Zones"—areas with high population density but low competitor saturation.',
    faq2ExplText: isAr
      ? 'أصبحت الرياض مشبعة بالصيدليات. يجب أن يحدد التحليل السليم كل منافس ضمن دائرة نصف قطرها 500 متر. تقوم S-LOC بأتمتة ذلك عن طريق رسم خرائط لجميع الفروع المنافسة فوراً ومقاطعتها مع القوة الشرائية الطبية للسكان.'
      : 'Riyadh is becoming saturated with pharmacies. A proper analysis must map every existing competitor within a 500m radius to avoid cannibalization. S-LOC automates this by instantly mapping all competitor branches and cross-referencing them with the medical spending power of the local residents.',
    faq2EvidText: isAr
      ? 'من خلال استخدام بيانات وزارة العدل ورسم الخرائط الجغرافية، توفر S-LOC "درجة موقع" معتمدة (0-100) تتنبأ بإمكانات الإيرادات اليومية لقطعة أرض معينة في الرياض.'
      : 'By utilizing MOJ transaction data and proprietary geospatial mapping, S-LOC provides a verified "Location Score" (0-100) that predicts the daily revenue potential of a specific plot in Riyadh.',
    faq2ActLink: isAr
      ? '[حلل موقع صيدليتك في الرياض على S-LOC]'
      : '[Analyze your Riyadh Pharmacy Site on S-LOC]',
    faq2ActText: isAr
      ? 'لترى ما إذا كان الموقع الذي اخترته "منطقة خضراء" أو "منطقة حمراء" على الفور.'
      : 'to see if your chosen location is a "Green Zone" or a "Red Zone" instantly.',

    faq3Title: isAr
      ? 'أي شركة يمكنها مساعدتي في عمل دراسة جدوى في جدة لافتتاح فرع جديد'
      : 'Which company can help me do feasibility study in Jeddah for opening a new branch',
    faq3StateText: isAr
      ? 'لافتتاح فرع جديد في جدة، تقدم S-LOC بديلاً أسرع وأكثر دقة في البيانات من الشركات الاستشارية التقليدية.'
      : 'For opening a new branch in Jeddah, S-LOC (S-Locator) offers a faster and more data-accurate alternative to traditional consulting firms.',
    faq3ExplText: isAr
      ? 'في حين تفرض الشركات الاستشارية التقليدية رسوماً باهظة لتقارير ثابتة تستغرق أشهراً، فإن S-LOC عبارة عن منصة توفر بيانات تفاعلية في الوقت الفعلي لتوسع التجزئة. وهي مصممة خصيصاً للتوسع متعدد الفروع، وتسمح بمقارنة 10 مواقع مختلفة في نفس الوقت.'
      : 'While traditional consulting firms in Jeddah charge high fees for static PDF reports that take months to prepare, S-LOC is a SaaS platform that provides real-time, interactive data for retail expansion. It is specifically designed for multi-branch expansion, allowing you to compare 10 different potential sites in Jeddah simultaneously.',
    faq3EvidText: isAr
      ? 'تتم مزامنة بيانات S-LOC مع الهيئة العامة للعقار وصندوق تنمية الموارد البشرية، مما يضمن توافق استراتيجيتك مع أحدث الاتجاهات الاقتصادية في جدة.'
      : 'S-LOC’s data is synchronized with the Real Estate General Authority (REGA) and HRDF, ensuring your expansion strategy aligns with the latest zoning and economic trends in Jeddah.',
    faq3ActLink: isAr ? '[استخدم S-LOC لتوسعك في جدة]' : '[Use S-LOC for Jeddah Expansion]',
    faq3ActText: isAr
      ? 'لإنشاء تقارير جدوى فورية بمستوى استثماري لفرعك الجديد.'
      : 'to generate immediate, investment-grade feasibility reports for your new branch.',
  }
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col lg:flex-row items-center gap-16 relative z-10">
          <div className={`lg:w-1/2 ${isAr ? 'text-right' : 'text-left'}`}>
            <FadeInSection>
              ...
              <p className="text-[#8492a6] text-lg mb-8 leading-relaxed max-w-lg">{t.heroDesc}</p>
            </FadeInSection>
            ...
            <FadeInSection delay={200}>
              ...
              <button
                className={`bg-[#38e54d] hover:bg-[#2ebf40] text-[#110222] font-bold py-3.5 px-8 rounded-full shadow-[0_4px_15px_rgba(56,229,77,0.3)] hover:-translate-y-1 transition-transform flex items-center gap-2 ${isAr ? 'flex-row-reverse' : ''} w-max`}
              >
          ...
          <div className="lg:w-1/2 relative w-full flex justify-center mt-12 lg:mt-0">
            <FadeInSection delay={400}>
              <div className="relative w-full max-w-[500px]">
                ...
                <div
                  className={`absolute -top-6 ${isAr ? '-left-6 md:-left-10' : '-right-6 md:-right-10'} bg-[#38e54d] text-[#110222] w-[120px] h-[120px] rounded-full flex flex-col justify-center items-center shadow-2xl border-[6px] border-[#0a2e16] z-20 hover:scale-105 transition-transform`}
                >
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col lg:flex-row gap-12 items-center lg:items-start">
          ...
          <div className={`lg:w-1/3 ${isAr ? 'text-right' : 'text-left'}`}>
            <FadeInSection>
              ...
              <button
                className={`bg-[#4ade80] hover:bg-[#38c966] text-[#110222] font-bold py-3 px-6 rounded-full shadow-[0_4px_15px_rgba(74,222,128,0.3)] hover:-translate-y-1 transition-transform flex items-center gap-2 text-sm w-max ${isAr ? 'flex-row-reverse' : ''}`}
              >
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row gap-16 items-center mb-24">
          <FadeInSection>
            <div className="flex flex-col gap-6 relative w-full md:w-[350px] items-center justify-center min-h-[350px]">
              ...
              />
              <div
                className={`w-36 h-36 bg-white shadow-2xl rounded-full flex items-center justify-center font-serif text-4xl text-[#2b1055] italic hover:-translate-y-2 transition-transform ${isAr ? 'md:mr-24' : 'md:ml-24'}`}
              >
                Touch
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 mt-20">
          <FadeInSection delay={100}>
            <AccordionItem title={t.faq1Title} isAr={isAr}>
              <p>
                <b className="text-[#38e54d]">{t.faq1State}</b> {t.faq1StateText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Expl}</b> {t.faq1ExplText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Evid}</b> {t.faq1EvidText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Act}</b>{' '}
          ...
          <FadeInSection delay={200}>
            <AccordionItem title={t.faq2Title} isAr={isAr}>
              <p>
                <b className="text-[#38e54d]">{t.faq1State}</b> {t.faq2StateText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Expl}</b> {t.faq2ExplText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Evid}</b> {t.faq2EvidText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Act}</b>{' '}
          ...
          <FadeInSection delay={300}>
            <AccordionItem title={t.faq3Title} isAr={isAr}>
              <p>
                <b className="text-[#38e54d]">{t.faq1State}</b> {t.faq3StateText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Expl}</b> {t.faq3ExplText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Evid}</b> {t.faq3EvidText}
              </p>
              <p>
                <b className="text-[#38e54d]">{t.faq1Act}</b>{' '}
